...
namespace Yaziji.LED
{
    public class LEDSendPackage : LEDPackege
    {
        ...
        private readonly static LEDContentAnimation DefaultLEDInfoAnimation = LEDContentAnimation.Display;
        private readonly static LEDContentStand DefaultLEDInfoStand = LEDContentStand.NoCircle;
        private readonly static int DefaultSpeed = 0x08;//0x00-0xff,数字越大越快
        //比如屏幕可以显示8个字，当前信息有16个字，那么前8个字移动完毕后，可以选择在屏幕上停留一会儿再显示后8个字。
        private readonly static int DefaultDuration = 0x08;//0x00-0xff,描述
        private readonly static int DefaultTextColor = 0x01;
        ...
        public override byte[] GetPackage()
        {
            using(MemoryStream ms = new MemoryStream())
	        {
                ...
                if (!string.IsNullOrEmpty(Content))//CommandExtend != null && CommandExtend.Length > 0)
                {
                    ...
                    //包内容
                    CommandExtend = Encoding.Unicode.GetBytes(Content);
                }
                //尾部
                Foot = new byte[4];
                //数据包总长度
